...
public class ChatCommands implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(!commandSender.hasPermission("arehard.core.chat")){
            commandSender.sendMessage(ChatUtil.fixColor("&8>> &4Blad: &cNie masz permsiji do &7(arehard.core.chat)"));
            return false;
        }
        if(strings.length == 0){
            commandSender.sendMessage(ChatUtil.fixColor("&8>> &4Blad: &cPoprawne uzycie: &7/chat <on|off|cc>"));
            return true;
        }
        switch(strings[0].toLowerCase()) {
            ...
            case "cc": {
                ...
                Bukkit.broadcastMessage(ChatUtil.fixColor("&8>> &7Chat zostal wyczyszcony"));
